...
@section('title', 'Kullanım Alanları | Bilyam')

@section('content')
    <div class="space-y-6">
        <section class="rounded-[2rem] border border-slate-200 bg-white p-6 shadow-soft dark:border-slate-800 dark:bg-slate-900">
            <h1 class="text-2xl font-semibold text-slate-900 dark:text-white">Kullanım Alanları</h1>
            <form method="POST" action="{{ route('admin.use-cases.store') }}" class="mt-6 grid gap-4 lg:grid-cols-4">
                @csrf
                <input type="text" name="name" placeholder="Kullanım alanı" class="rounded-2xl border-slate-300 text-sm dark:border-slate-700 dark:bg-slate-950 dark:text-white" required />
                ...
                <button type="submit" class="lg:col-span-4 rounded-full bg-bilya-blue px-4 py-3 text-sm font-semibold text-white transition hover:bg-bilya-navy">Kullanım Alanı Ekle</button>
            </form>
        </section>
        <section class="space-y-4">
            ...
            @empty
                <div class="rounded-[2rem] border border-dashed border-slate-300 bg-white p-10 text-center text-sm text-slate-500 dark:border-slate-700 dark:bg-slate-900 dark:text-slate-400">Henüz kullanım alanı kaydı yok.</div>
            @endforelse
        </section>
    </div>
@endsection
