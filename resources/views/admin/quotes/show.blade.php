...
@section('title', $quote->quote_no.' | Teklif Detayi')
        ...
        <section class="rounded-[2rem] border border-slate-200 bg-white p-6 shadow-soft dark:border-slate-800 dark:bg-slate-900">
            <div class="flex flex-col gap-4 xl:flex-row xl:items-start xl:justify-between">
                ...
                <form method="POST" action="{{ route('admin.quotes.status.update', $quote) }}" class="grid gap-3 sm:grid-cols-[200px_minmax(0,1fr)_auto] xl:w-[760px]">
                    ...
                    <button type="submit" class="rounded-full bg-bilya-blue px-4 py-3 text-sm font-semibold text-white transition hover:bg-bilya-navy">Durumu Guncelle</button>
                </form>
            </div>
            @error('status')
            ...
            <div class="space-y-6">
                <section class="rounded-[2rem] border border-slate-200 bg-white p-6 shadow-soft dark:border-slate-800 dark:bg-slate-900">
                    ...
                    <div class="mt-4 space-y-3 text-sm text-slate-600 dark:text-slate-300">
                        ...
                        <div class="flex items-center justify-between"><span>Musteri</span><span class="font-semibold text-slate-900 dark:text-white">{{ $quote->customer?->name ?: $quote->requester_name }}</span></div>
