        ...
        <div class="mx-auto max-w-7xl space-y-6 sm:px-6 lg:px-8">
            <section class="marketing-surface soft-grid p-6 lg:p-8">
                <div class="mb-5 flex flex-col gap-3 lg:flex-row lg:items-end lg:justify-between">
                    <div>
                        <p class="eyebrow">Teklif Geçmişi</p>
                        <h2 class="mt-2 text-3xl font-semibold text-slate-900 dark:text-white">Tüm taleplerinizi tek ekranda takip edin</h2>
                    </div>
                    <a href="{{ route('quote-list.index') }}" class="cta-primary">Yeni Teklif Oluştur</a>
                </div>
                <form method="GET" action="{{ route('account.quotes.index') }}" class="grid gap-4 lg:grid-cols-5">
                    ...
                    <button type="submit" class="cta-primary px-4 py-3">Filtrele</button>
                </form>
            </section>
                ...
                @else
                    <div class="space-y-3">
                        @foreach ($quotes as $quote)
                            <a href="{{ route('account.quotes.show', $quote) }}" class="flex flex-col gap-4 rounded-[1.5rem] bg-slate-50 px-5 py-4 transition hover:text-bilya-blue dark:bg-slate-950 lg:flex-row lg:items-center lg:justify-between">
                                <div>
