...
<html lang="{{ str_replace('_', '-', app()->getLocale()) }}" class="h-full">
    ...
    <body class="min-h-full bg-slate-100 text-slate-900 antialiased dark:bg-slate-950 dark:text-slate-100">
        <div class="grid min-h-screen lg:grid-cols-[1.05fr_minmax(0,460px)]">
            <div class="hidden bg-bilya-navy px-10 py-12 text-white lg:flex lg:flex-col lg:justify-between">
                <div class="space-y-8">
                    <a href="{{ route('home') }}">
                        <x-application-logo class="h-12 w-auto" />
                    </a>
                    <div class="space-y-4">
                        <p class="inline-flex rounded-full border border-white/20 px-4 py-1 text-xs font-semibold uppercase tracking-[0.24em] text-white/80">B2B Agirlikli Katalog</p>
                        <h1 class="max-w-xl text-4xl font-semibold leading-tight">Teknik urunleri kesfedin, teklif akisini tek merkezde yonetin.</h1>
                        <p class="max-w-lg text-base text-slate-200">Bilyam, ilk fazda katalog + teklif listesi + musteri paneli + admin operasyonu omurgasiyla kuruluyor.</p>
                    </div>
                </div>
                <div class="grid gap-4 text-sm text-slate-200">
                    <div class="rounded-2xl border border-white/10 bg-white/5 p-4">Misafir teklif akisi acik, uyelik ise teklif takibini kolaylastiran deger katmani olarak konumlanir.</div>
                    <div class="rounded-2xl border border-white/10 bg-white/5 p-4">Light/dark tema destegi ve mobil dogal deneyim altyapisi bu asamada hazirlandi.</div>
                </div>
            </div>
            <div class="flex items-center justify-center px-4 py-10 sm:px-6 lg:px-10">
                <div class="w-full max-w-md">
                    ...
                    <div class="rounded-3xl border border-slate-200 bg-white p-8 shadow-soft dark:border-slate-800 dark:bg-slate-900">
                        {{ $slot }}
                    </div>
                </div>
            </div>
